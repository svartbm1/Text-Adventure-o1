...
import o1.adventure.MyAdventure
...
object AdventureGUI extends SimpleSwingApplication {
  ...
  def top = new MainFrame {
    ...
    val game = new MyAdventure
    val player = game.player
    
    // Components: 

    val locationInfo = new TextArea(8, 120) {
      editable = false
      wordWrap = true
      lineWrap = true
    }
    val turnOutput = new TextArea(16, 120) {
      editable = false
      wordWrap = true
      lineWrap = true
    }
    val input = new TextField(60) {
    ...
    def updateInfo(info: String) = {
      var alert = ""
      game.checkIfDetected
      if (player.isDetected) alert = "\n" + player.guardReport
      if (!this.game.isOver) {
        this.turnOutput.text = info
      } else {
        this.turnOutput.text = info + "\n\n" + this.game.goodbyeMessage
      }
      this.locationInfo.text = this.player.location.fullDescription + alert
