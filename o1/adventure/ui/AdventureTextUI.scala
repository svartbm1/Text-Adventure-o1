...
object AdventureTextUI extends App {
  ...
  private val game = new MyAdventure
  ...
  private def run() = {
    ...
    this.printAreaInfo() //added this so it would be like the GUI
  ...
  private def playTurn() = {
    ...
    if (player.isDetected) println(player.guardReport)
    val command = readLine("Command: ")
    val turnReport = this.game.playTurn(command)
    if (!turnReport.isEmpty) { //if error here check no action is returning anything but string
      println(turnReport) 
    }
  }
